...
2. 브라우저(Playwright, executablePath: '/opt/pw-browsers/chromium')로 수정한 도구를 직접 열어 콘솔 오류가 없는지, 핵심 흐름이 동작하는지 확인한다.
3. petfocus를 고쳤다면 데이터 규칙(records 재계산·soft delete·스키마 버전)과 원가 검산(231,000원 / 개당 4,620원)을 확인한다.
